...
in term of distance between codons in the framework set in reference (1). The
decision tree is build based on Chi-squared Automated Interaction Detection 
(CHAID) algorithm.
...
two aminocid distances, which are based on the codon distance.
...
datos$mut_effect <- round((aminoacid_dist(aa1 = h_autby_coef, group = "Z5",
...
```{r rule2.}
rule <- (dat$mut_effect == 0.28 & dat$regions == "R4" & dat$autm == "A4")
table(as.character(dat[ rule, ]$species))
```

Only gorilla hold the following rule
```{r rule3.}
rule <- (dat$mut_effect == 1.36 )
table(as.character(dat[rule,]$species))
```
